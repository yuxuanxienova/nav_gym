...
#-------- 1. Initialize the environment--------
    def __init__(self, cfg: LocomotionASEEnvCfg):
    ...
    def _init_buffers(self):
        ...
        self.dof_pos_history = torch.zeros(
            self.num_envs, self.history_length, self.robot.num_dof, dtype=torch.float, requires_grad=False
        ).to(self.device)#(num_envs, history_length, num_dof)
