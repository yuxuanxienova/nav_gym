...
#-------- 1. Initialize the environment--------
    ...
    def _init_buffers(self):
        ...
        #----latent commands
        self.command_phase_sin = torch.zeros(self.num_envs, self.cfg.fld.latent_channel,device=self.device)
        self.command_phase_cos = torch.zeros(self.num_envs, self.cfg.fld.latent_channel,device=self.device)
        self.command_freq = torch.zeros(self.num_envs, self.cfg.fld.latent_channel,device=self.device)
        self.command_amp = torch.zeros(self.num_envs, self.cfg.fld.latent_channel,device=self.device)
        self.command_offset = torch.zeros(self.num_envs, self.cfg.fld.latent_channel,device=self.device)
        self.command_residual = torch.zeros(self.num_envs, self.num_actions,device=self.device)
...
#-------- 2. Reset the environment--------
    ...
    def reset_idx(self, env_ids):
        ...
        # ).squeeze()
        #----latent commands
        self.command_phase_sin[env_ids,:] = 0.0 
        self.command_phase_cos[env_ids,:] = 0.0 
        self.command_freq[env_ids,:] = 0.0 
        self.command_amp[env_ids,:] = 0.0 
        self.command_offset[env_ids,:] = 0.0 
        #--------reset other modules-------
...
#-------- 4. Get/Set functions--------
    ...
    def set_commands(self, commands:dict):
        ...
        command_freq = commands["freq"]
        command_amp = commands["amp"]
        command_offset = commands["offset"]
        command_residual = commands["residual"]
        #process command
        self.command_phase_sin = torch.sin(2 * torch.pi * command_phase)
        self.command_phase_cos = torch.cos(2 * torch.pi * command_phase)
        self.command_freq = command_freq
        self.command_amp = command_amp
        self.command_offset = command_offset
        self.command_residual = command_residual
    ...
    def set_observation_buffer(self):
        ...
        obs_list.append(self.command_freq)
        obs_list.append(self.command_amp)
        obs_list.append(self.command_offset)
