...
from nav_gym.nav_legged_gym.common.commands.command import CommandBase,UnifromVelocityCommand,UnifromVelocityCommandCfg,WaypointCommandCfg,WaypointCommand
...
#-------- 4. Get/Set functions--------
    ...
    def set_observation_buffer(self):
        ...
        obs_list = []
        #1. prop
        obs_list.append(self.obs_manager.obs_per_func["goal_position_commands"])#add velocity commands
...
#-------- 5. Other functions--------
    ...
    def _draw_target_position(self):
        self.sphere_geoms_red = BatchWireframeSphereGeometry(num_spheres=1,radius=0.1, num_lats=4, num_lons=4, pose=None, color=(1, 0, 0))
        self.sphere_geoms_red.draw(self.command_generator.get_goal_position_command()[0], self.gym, self.viewer, self.envs[0])
