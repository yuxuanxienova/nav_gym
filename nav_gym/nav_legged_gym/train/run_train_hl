...
from nav_gym.learning.modules.VAE import VAE,VariationalEncoder
import torch
import os
import time
...
if __name__ == "__main__":
    ...
    ll_policy = runner.get_inference_policy()
    ...
    latent_dim = 3
    hidden_dim = 256
    hl_policy = VariationalEncoder(input_dim, latent_dim, hidden_dim)
    hl_policy.to(device)

    # Define the optimizer
    learning_rate = 1e-4  # Adjust the learning rate if necessary
    optimizer = optim.Adam(hl_policy.parameters(), lr=learning_rate,weight_decay=1e-5)

    # Initialize a step counter
    global_step = 0
    while True:
        # -----------Step Low Level Policy------------
        #obs:Dim(num_envs,obs_dim)
        action = ll_policy(obs)
        #action:Dim(num_envs,action_dim)
        obs, _, _, _, extras = env.step(action)
        env.render()

        # ------------Get High Level Policy------------
        obs_pc = env.obs_manager.get_obs_from_group("point_cloud")
        # obs_pc:Dim(num_envs, n_points*3)
        if torch.isnan(obs_pc).any():
            print("Warning: obs contains NaN values!")
            continue
        action = hl_policy(obs_pc)
