...
class UnifromVelocityCommandCfg:
    ...
    prob_heading_envs = 0.8  # percentage of the robots follow heading command (the others follow angular velocity)
    prob_zero_dim = 0.05

    @configclass
    class Ranges:
        lin_vel_x: Tuple = (-1.0, 2.0)  # min max [m/s]
        lin_vel_y: Tuple = (-0.3, 0.3)  # min max [m/s]
...
@configclass
class UnifromVelocityCommandCfg_LocomotionPAE:
    class_name: str = "UnifromVelocityCommand"
    robot_name: str = "robot"
    curriculum = False
    max_curriculum = 1.0
...
class WaypointCommandCfg2:
    ...
    class Ranges:
        radius_range: Tuple = (15.0, 20.0)  # min max dist [m]
        heading_range: Tuple = (-3.14, 3.14)  # [rad]
        max_velocity: Tuple = (2.0, 1.0, 1.5)
...
@configclass
class WaypointCommandCfg_LocomotionPAE:
    class_name: str = "WaypointCommand"
    robot_name: str = "robot"
    resampling_time: float = 15.0  # time before commands are changed [s]
    resampling_prob: float = 0.1  # probability of resampling a new waypoint
    ...
    class Ranges:
        radius_range: Tuple = (6.0, 17.0)  # min max dist [m]
        heading_range: Tuple = (-3.14, 3.14)  # [rad]
        max_velocity: Tuple = (1.0, 1.0, 1.3)
