...
class AMPObsStorage:
    def __init__(self,obs_dim,num_envs,num_transitons_per_env, trajectory_capacity):
        ...
        #initialize buffers
        self.transition_buffer = torch.zeros(num_transitons_per_env,num_envs,obs_dim)
        self.data = torch.zeros(trajectory_capacity,num_transitons_per_env,num_envs,obs_dim)
        self.count = 0
        self.length = 0
    ...
    def clear_buffer(self):
        self.transition_buffer = torch.zeros(self.num_transitons_per_env,self.num_envs,self.obs_dim)
    ...
    def get_current_obs(self,sample_length):
        ...
        if self.num_transitons_per_env < sample_length:
            ...
            exit(1)
        #--------------------
        #self.data[self.count]: [num_transitons_per_env,num_envs,obs_dim]
        obs_amp = self.data[self.count][:sample_length].swapaxes(0,1)
    ...
    def sample_amp_obs_batch(self, sample_length, num_sample=1):
        ...
        #self.data[indices]: [num_sample,num_transitons_per_env,num_envs,obs_dim]
        obs_amp_replay = self.data[indices][:,:sample_length].swapaxes(1,2).flatten(0,1)  
