...
class FLDLocomotionActorCritic(nn.Module):
    def __init__(self, obs_shape_dict, action_size, cfg, **kwargs):
        ...
        self.num_obs = self.prop_size + self.exte_size + self.priv_size + self.fld_obs_size
        ...
        self.action_head = MLP(
            cfg["output_mlp_size"],
            self.activation_fn,
            self.priv_latent_size + cfg["scan_latent_size"] + self.prop_size + self.fld_obs_size,
            self.action_size,
            init_scale=1.0 / np.sqrt(2),
        )

    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        prop, exte, priv, fld = self.split_obs(obs)
        ...
        observation_latent = torch.cat([prop, exte_latent, priv_latent, fld], dim=1)
        output = self.action_head(observation_latent)
        return output

    def split_obs(self, obs):
        return torch.split(obs, [self.prop_size, self.exte_size, self.priv_size, self.fld_obs_size], dim=1)
