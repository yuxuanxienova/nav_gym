...
class NavPolicyWithMemory(SimpleNavPolicy):
    def __init__(self, obs_shape_dict, action_size, cfg, **kwargs):
        ...
        self.aggregator = SimplePointNet([self.num_nodes, self.memory_shape[1]],
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        prop, scan,  history, memory = self.split_obs(obs)
        ...
        memory_feature = memory.reshape(obs.shape[0], self.num_nodes, -1)
        pointnet_feature = self.aggregator(memory_feature)

        concat_feature = torch.cat([scan_latent, prop, history_latent, pointnet_feature], dim=1)
        output = self.action_head(concat_feature)
        return output
