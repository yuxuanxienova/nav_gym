...
class PPO_AMP:
        ...
        for (
            ...
            if self.amp_obs_storage.is_ready(self.history_length):
                ...
                if self.amp_normalizer is not None:
                    with torch.no_grad():
                        obs_amp = self.amp_normalizer.normalize_torch(obs_amp, self.device)
                        # obs_amp_replay = self.amp_normalizer.normalize_torch(obs_amp_replay, self.device)
                        obs_demo = self.amp_normalizer.normalize_torch(obs_demo, self.device)
    ...
    def _disc_loss(self, disc_agent_logit, disc_demo_logit, obs_demo):
        ...
        #--------------------loss2---------------------
        disc_loss_agent = torch.nn.MSELoss()(disc_agent_logit,-1 * torch.ones(disc_agent_logit.size(), device=self.device))
        disc_loss_demo = torch.nn.MSELoss()(disc_demo_logit,torch.ones(disc_demo_logit.size(), device=self.device))
        disc_loss = 0.5 * (disc_loss_agent + disc_loss_demo)
